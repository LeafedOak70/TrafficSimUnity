...
public class WFC: MonoBehaviour{
    ...
    void GenerateGrid(){
        for(int x = 0;x < dimensions; x++){
            for(int y = 0;y < dimensions; y++){
                ...
                newCell.CreateCell(false, x, y, tileObjects, index);
    ...
    IEnumerator CheckEntropy(){
        List<Cell> tempGrid = new List<Cell>(gridCells);
        //Remove all collapsed cells and sort remaining by lowest entropy
        tempGrid.RemoveAll(c => c.collapsed);
        tempGrid.Sort((a, b) => { return a.tileOptions.Length - b.tileOptions.Length; });

        //Find what point cells get higher entropy than the lowest, 
        //that way the list can only have the lowest entropy cells
        int lowestEntropy = tempGrid[0].tileOptions.Length;
        int stopIndex = 0;
        for(int i =1;i < tempGrid.Count;i++){
            if(tempGrid[i].tileOptions.Length > lowestEntropy){
                stopIndex = i;
                break;
            }
        }
        //Remove all the cells with higher entropy
        if(stopIndex > 0){
    ...
    {
        List<Cell> newGenerationCell = new List<Cell>(gridCells);
        ...
        for (int y = 0; y < dimensions; y++)
        {
            for (int x = 0; x < dimensions; x++)
            {
                ...
                else
                {
                    ...
                    if (y > 0)
                    {
                        Cell up = gridCells[x + (y - 1) * dimensions];
                    ...
                    if (x < dimensions - 1)
                    {
                        Cell right = gridCells[x + 1 + y * dimensions];
                        List<Tile> validOptions = new List<Tile>();
                    ...
                    if (y < dimensions - 1)
                    {
                        Cell down = gridCells[x + (y + 1) * dimensions];
                        List<Tile> validOptions = new List<Tile>();
                    ...
                    if (x > 0)
                    {
                        Cell left = gridCells[x - 1 + y * dimensions];
                        List<Tile> validOptions = new List<Tile>();
                    ...
                    newGenerationCell[index].setTiles(newTileList);
                }
            }
        }
